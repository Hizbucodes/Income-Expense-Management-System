...
const EditForm = ({
  ...
  const incomeDetails = incomeData.find(
    (incomedtl) => incomedtl.id.toString() === id
  );

  useEffect(() => {
    if (incomeDetails) {
      setEditTitle(incomeDetails.title);
      setEditAmount(incomeDetails.amount);
      setEditCategory(incomeDetails.category);
      setEditDate(incomeDetails.date);
      setEditDescription(incomeDetails.description);
    }
  }, [
    incomeDetails,
    setEditTitle,
  ...
  return (
    <div className="w-[80%] text-center border-2 pb-2 rounded-lg">
      ...
      >
        ...
        <button
          type="submit"
          onClick={() => handleEdit(incomeDetails.id)}
